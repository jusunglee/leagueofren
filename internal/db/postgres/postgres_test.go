...
func newTestRepo(t *testing.T) *Repository {
	...
	if dsn == "" {
		dsn = "postgres://leagueofren:localdev123@localhost:5432/leagueofren?sslmode=disable"
	}
	repo, err := New(context.Background(), dsn)
	require.NoError(t, err)
	t.Cleanup(func() {
		repo.pool.Exec(context.Background(),
			"TRUNCATE subscriptions, evals, translations, translation_to_evals, feedback, riot_account_cache, riot_game_cache CASCADE")
		repo.Close()
	})
	return repo
}
...
func TestGameCache(t *testing.T) {
	...
	err := repo.CacheGameStatus(ctx, db.CacheGameStatusParams{
		...
		Participants: []byte(`[{"puuid":"p1","riotId":"P1#NA1"}]`),
	})
	require.NoError(t, err)
